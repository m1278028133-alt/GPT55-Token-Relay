-- GPT-5.5 Token Relay database initialization.
-- Run this file in Supabase SQL Editor.
-- Before running the optional admin insert at the bottom:
-- 1. Create an Auth user in Supabase Dashboard.
-- 2. Copy the Auth user UUID.
-- 3. Replace the placeholder UUID and email.
...
begin
  ...
  on conflict (user_id)
  do update set balance = public.token_balances.balance + p_tokens, updated_at = now();
end;
$$;
...
begin
  ...
  set total_deposited = total_deposited + p_amount_usd,
...
begin
  update public.fund_pool
  set total_spent = total_spent + p_amount_usd,
      current_balance = current_balance - p_amount_usd,
      last_updated = now()
  where id = (select id from public.fund_pool limit 1)
    and current_balance >= p_amount_usd;
  if not found then raise exception 'fund pool balance is insufficient'; end if;
end;
$$;
...
declare v_from text;
begin
  select upstream_service into v_from from public.upstream_config where is_primary = true limit 1;
  update public.upstream_config set is_primary = false, updated_at = now() where is_primary = true;
  update public.upstream_config set is_primary = true, enabled = true, updated_at = now() where upstream_service = p_upstream_service;
...
-- Optional initial admin profile:
-- Create the Auth user first in Supabase Dashboard, then replace these placeholders.
-- admin UUID: f5922359-53c3-4009-8aea-aece06060daa
-- admin email: 1278028133@qq.com
--
insert into public.users (id, email, role)
values ('f5922359-53c3-4009-8aea-aece06060daa', '1278028133@qq.com', 'admin')
